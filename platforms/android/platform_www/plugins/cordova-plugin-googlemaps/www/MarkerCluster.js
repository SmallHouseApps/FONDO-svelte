...
MarkerCluster.prototype.onClusterClicked = function(cluster) {
  ...
  if(bounds.northeast.lat != bounds.southwest.lat && bounds.southwest.lng != bounds.northeast.lng){
    if (self.boundsDraw) {
      polygon.setPoints([
        bounds.southwest,
    ...
    bounds.northeast.lat = parseFloat(bounds.northeast.lat) + parseFloat(0.05);
    bounds.northeast.lng = parseFloat(bounds.northeast.lng) + parseFloat(0.02);
    ...
    if(window.ctrl && window.ctrl.map) window.ctrl.map.set({ cluster: bounds });
  }
};
