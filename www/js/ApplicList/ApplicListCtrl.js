(function (global, factory) {
    ...
    let equalSignSY = 0; let negativeSY = 0, positiveSY = 0;
    let main = document.getElementById('main'); let dateChange;

    let nav = document.getElementsByClassName('nav-extended')[0]; let delay;
    let popUp = document.getElementById('pop-up'); let currentApplicId, currentOrderId;
    let notFilterApplics; let headerOffsetTop = 0; let loader = 1;
    let applicsCount = {
        all: 0,
        done: 0,
        progress: 0,
    ...
    let self = this; let tempStorageApplics = []; let startStateApplics;
    let storageApplics = JSON.parse(localStorage.getItem('applics')); let prevOrderId = null;
    let preloader = null; let prevSY = null; let sydif = 0; let sydifrepeat = 0;
    let applicCount = 0;
    var now = new Date();
    ...
    function updateVisible() {
        // let applics = window.ctrl.applicList.get('applics'); let beforeOrderId;
                ...
                .then(r => {
                    if (r.status == 200) {
                        r.json().then(data => {
                            ...
                            let step = preloader.next();
                            let applics = data.data.map(function (applic, index) {
    ...
    let ApplicListCtrl = new SvelteController(ApplicListComponent, {
        ...
        observes: [{
            ...
            function: currentCategory => {
                if (window.ctrl.applicList) {
                    ...
                    let applics = ApplicListCtrl.get('applics');
        ...
        methods: [{
            ...
            function: async (el, applicId) => {
                let data = ApplicListCtrl.get('applics').map(function (applic) {
            ...
            function: (searchText, global) => {
                ...
                let applics = notFilterApplics.map(function(applic){
            ...
            function: async orderIds => {
                let applics = notFilterApplics.filter(applic => orderIds.indexOf(applic.order_id) != -1);
                refresh(applics);
            }
        }, {
            name: 'showApplic',
            ...
            function: async (applicId, e, fromApplic = false) => {
                console.log(e);
                let applic = ApplicListCtrl.get('applics').filter(applic => applic.id == applicId)[0];
            ...
            function: async (applicId, el) => {
                let messageEl = document.getElementById('applic-messages' + applicId);
                let content = document.getElementById('applic-messages-content' + applicId);
            ...
            function: (items, weight, comment) => {
                let serverChange = {};
                let itemsIssuedCount = 0;
                serverChange.applicId = currentApplicId;
                serverChange.items = [];
                serverChange.weight = weight;
                serverChange.comment = comment;

                let currentApplic = ApplicListCtrl.get('applics').filter(applic => applic.id == currentApplicId)[0];
                ...
                let tempApplicsCount = window.ctrl.header.get('applicsCount');
                ...
                let category = ApplicListCtrl.get('currentCategory');
