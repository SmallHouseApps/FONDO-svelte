(function (global, factory) {
    ...
    'use strict';
    let modal = document.getElementById('modal'); let timer;
    let cover = document.getElementById('cover'); let startScrollLeft;
    ...
    document.getElementById('menu').addEventListener('touchend', function(e){
        if(e.changedTouches[0].clientX < startScrollLeft){
            ...
            let start = document.getElementById('menu').scrollLeft;
    ...
    async function preloader() {
        ...
        clearInterval(timer);
        function tick(){
            let workTime = new Date(Date.now() - window.ctrl.work.getWorkShift().timestamp_start);

            let h = workTime.getHours() - 3; h = h < 10 ? ( '0' + h ) : h;
            let m = workTime.getMinutes(); m = m < 10 ? ( '0' + m ) : m;
            let s = workTime.getSeconds(); s = s < 10 ? ( '0' + s ) : s;
    ...
    let MenuCtrl = new SvelteController(MenuComponent, {
        globalName: 'menu',
