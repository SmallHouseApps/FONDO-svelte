(function (global, factory) {
    ...
    let modal = document.getElementById('modal'); let issuedCount = 0;
    let cover = document.getElementById('cover'); 
    let mainEl;
    let instance;
    let startClientX = null;
    let tempData = JSON.parse(JSON.stringify(window.ctrl.tempData));
    ...
    let ItemModalCtrl = new SvelteController(ItemCardsComponent, {
        preloader: preloader,
        ...
        methods:[{
            ...
            function: async (itemId, index) => {
                if(itemId){
                    let currentItem;
                    let extra = ItemModalCtrl.get('extra');
                    let items = ItemModalCtrl.get('items').map(function(item){
                        if(item.id == itemId) {
                    ...
                    if(currentItem.quantity == 1 || currentItem.issuedPart){
                        if(issuedCount == items.length && extra.issued){
                            if(extra.weight != '' && extra.comment != ''){
                                ...
                                window.ctrl.applicList.component.complete(items, extra.weight, extra.comment);
                    ...
                    let items = ItemModalCtrl.get('items');
            ...
            name: 'showDenialBtns',
            function: () => {
                let el = document.getElementById('denial-block');
