(function (global, factory) {
    ...
    let mapFocus = false;
    let touchstart, animateCamera;
    var main = document.getElementById('main');
    let nav = document.getElementsByClassName('nav-extended')[0];
    let map, onReadyIconsLink;
    let mapApplics;
    let dlMarkers = [], tkMarkers = [];
    let dlCluster, tkCluster; let firstLoad = true;
    let icons = {
    ...
    function showOnMap(applics) {
        refresh();
        let self = this; 
        let bounds, prevOrderId, prevOrderIdDeliveryCoords = {}, polylineCoords = [];
    ...
    function preapareMarkerIcon(type) {
        let self = this, color, icon, canvas, context, img;
    ...
    let MapCtrl = new SvelteController(MapComponent, {
        globalName: 'map',
        onCtrlReady: async () => {
            ...
            function: async cluster => {
                if(cluster){
                    ...
                    if (northeast.lat == southwest.lat && northeast.lng == southwest.lng) {
                        ...
                        main.style.setProperty('transform', 'translate3d(0, 0, 0)');
                        main.style.setProperty('height', '50vh');
                    }
                }
            }
        }],
